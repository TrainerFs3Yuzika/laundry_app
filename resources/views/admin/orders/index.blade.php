                ...
                <tr>
                    ...
                    <td class="text-center border-b">
                        @if ($order->status == 'paid')
                            <div class="py-2 px-2 rounded-full text-xs bg-green-600 text-white cursor-pointer font-medium">
                                {{ ucfirst($order->status) }}
                            </div>
                        @elseif ($order->status == 'pending')
                            <div class="py-2 px-2 rounded-full text-xs bg-theme-11 text-white cursor-pointer font-medium">
                                {{ ucfirst($order->status) }}
                            </div>
                        @else
                            <div class="py-2 px-2 rounded-full text-xs bg-theme-6 text-white cursor-pointer font-medium">
                                {{ ucfirst($order->status) }}
                            </div>
                        @endif
                    </td>
                        ...
                        <div class="flex sm:justify-center items-center">
                            @if ($order->status == 'paid')
                                ...
                                @elseif ($order->status_order == 'completed')
                                    <span class="text-gray-600">Order Completed</span>
                                @endif
                            @else
...
<script>
    $(document).ready(function() {
        // Initialize DataTable with default sorting on the first column in descending order
